...
@Component({
  selector: 'app-add-more-education-details',
  templateUrl: './add-more-education-details.component.html',
  styleUrls: ['./add-more-education-details.component.css']
})
export class AddMoreEducationDetailsComponent {
  constructor(
  ...
  displayedColumns: string[] = ['position', 'name_seeker','company','specialization','gender','applied_date','status'];
  ...
  saveMoreEducation(){
    ...
    this.registrationService.getSpecificUserInformation(sessionStorage.getItem('login_id')).subscribe((resp:any)=>{
      this.seeker_service.getSeekerByUserInformationID(resp.user_information_id).subscribe((response:any)=>{
        this.education_details.seeker_id = response.seeker_id;
        this.seeker_service
        ...
        .subscribe((resp: any) => {
          ...
          Toast.fire({
            icon: 'success',
            title: 'Your education information has been added successfully',
          });
          this.route.navigate(["job_portal/applicant_education_details"]);
        });
      });
    });
  }
