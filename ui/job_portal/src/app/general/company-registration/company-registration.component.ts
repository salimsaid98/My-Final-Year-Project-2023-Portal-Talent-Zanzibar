...
import { CompanyImage } from 'src/app/models/company-image';
import { CompanyModel } from 'src/app/models/company-model';
import { CompanySpecialization } from 'src/app/models/company-specialization';
...
export class CompanyRegistrationComponent {
  ...
  company_model: CompanyModel = new CompanyModel();
  company_image: CompanyImage = new CompanyImage();
  login_model: LoginModel = new LoginModel();

  company_specialization: CompanySpecialization = new CompanySpecialization();
  ...
  getAllSpecializations() {
    this.company_spec_service
      .getAllSpecializations()
      .subscribe((output: any) => {
        this.comp_spec_data = output;
      });
  }
  ...
  saveCompany() {
    this.login_model.role = 'Company';
    this.company_model.email_address = this.login_model.username;
    this.login_service.user_registrations(this.login_model).subscribe(
      (output: any) => {
        this.company_model.user_id = output.user_id;
        this.company_services.saveCompany(this.company_model).subscribe(
          (output_2: any) => {
            const form1 = new FormData();
            form1.append('imageFile', this.Selectfile, this.Selectfile.name);
            this.company_services.saveCompanyLogo(form1,output_2.company_id).subscribe((resp:any)=>{});
            for (const element of this.company_specialization.specialization_id) {
              const c_spec = new CompanySpecialization();
              c_spec.company_id = output_2.company_id;
              c_spec.specialization_id =
                element;
              this.company_spec_service
                .saveCompanySpecialization(c_spec)
                .subscribe((saved_: any) => {});
            }
            this.router.navigate(['../../job_portal/login_page']);
            this.successAlert();
          },
  ...
  error_alert(){
    Swal.fire({
      icon: 'error',
      title: 'Oops...',
      text: 'Something went wrong! user already exists',
    })
  }
  ...
  successAlert() {
    ...
    Toast.fire({
      icon: 'success',
      title: 'Company Registration successfully',
    });
  }
}
