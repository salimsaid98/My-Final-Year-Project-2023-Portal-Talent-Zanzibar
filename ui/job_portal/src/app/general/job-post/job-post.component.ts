...
import { JobLocation } from 'src/app/models/job-location';
import { JobPost } from 'src/app/models/job-post';
import { JobSkills } from 'src/app/models/job-skills';
...
export class JobPostComponent {
  ...
  job_post: JobPost = new JobPost();
  job_location: JobLocation = new JobLocation();
  job_skills: JobSkills = new JobSkills();
  ...
  saveJobPost(){
    ...
    this.company_service.getCompanyInformationUsingUserId(sessionStorage.getItem('login_id')).subscribe((output:any)=>{
      this.job_post.posted_by = output.company_name;
    });

    let newdate = year + "/" + month + "/" + day;
    this.job_post.starting_date = newdate;
    this.job_post.company_id = sessionStorage.getItem("company_id");

    this.job_service.saveJobLocation(this.job_location).subscribe((output:any)=>{
      this.job_post.job_location_id = output.job_location_id;
        this.job_service.saveJobPosts(this.job_post).subscribe((resp:any)=>{
            this.job_skills.job_post_id = resp.job_post_id;
            for (const element of this.job_skills.skill_id) {
              this.job_skills.skill_id = element;
              this.job_service.saveJobSkill(this.job_skills).subscribe((response:any)=>{
                this.success_alert();
                this.router.navigate(["job_portal/job_list"]);
              });
            }
        });
    });
  ...
  getAllSkill(){
    this.job_service.getSkills().subscribe((output:any)=>{
      this.skill_data = output;
    });
  }

  getAllJobTypes(){
    this.job_service.getAllJobTypeList().subscribe((output:any)=>{
      this.job_type_data = output;
    })
  }
  ...
  error_alert(){
    ...
    Toast.fire({
      icon: 'error',
      title: 'Error occured while posting job'
    })
  }
